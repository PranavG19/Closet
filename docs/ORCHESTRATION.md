...
4. **Don't SendMessage-resume an agent that a workflow is still orchestrating.** The W3 build agent lives inside workflow wf_4520cb7a-a8d; resuming it via SendMessage risks two execution threads on one worktree. In practice it stayed single-writer (verified via monotonic file mtimes — no race), but the agent got briefly confused seeing its own output accumulate. Prefer: let the workflow drive; only message an agent that has no active task. If you must correct a running workflow agent, verify single-writer via mtimes before assuming a race, and check at integration that the worktree is one coherent build.
5. **Integration tests MUST live under `packages/*/test/`, not `src/`.** The vitest `integration` project globs `packages/*/test/**/*.integration.test.ts`; a `.integration.test.ts` under `src/` is SILENTLY skipped (proves nothing while looking green). Task files that place integration tests under src/ are wrong — builders put them under `packages/functions/test/`. (Caught by the W3 build agent.)
