...
| **Fix the 6 visual defects** | ◐ **5 of 6 coded, 0 of 6 re-captured** | Safe-area insets + tab label (`aa025e9`); the paywall price + Restore control; the AA palette (accent split by role, hues preserved, now guarded by `tokens/contrast.test.ts`); `typography.family` is required and set. Remaining: the simulator gear overlay, which is not app code — those 17 PNGs simply cannot be store assets. **No fix is visually confirmed** — every PNG is pre-fix, and a re-capture is the only oracle that closes a row. |
