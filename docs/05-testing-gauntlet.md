...
> **✅ PARTIAL HARNESS LANDED (2026-08-11).** The measurement primitive and the first two lanes now exist. `packages/db/test/helpers/perf.ts` provides `measure()` (serial, `process.hrtime.bigint`) and `measureConcurrent(total, concurrency, opFor)` (a fixed worker-pool load generator), both unit-oracled in `perf.test.ts` (nearest-rank percentile math; and that the load generator runs each op exactly once and never exceeds the concurrency cap — so "under load" is not a serial loop in disguise). Two `*.perf.test.ts` lanes consume it: the **DB-only** lane (`packages/db/test/repos.perf.test.ts`, 10 repo ops, SLO-gated) and the **full-stack API load** lane (`packages/functions/test/api-load.perf.test.ts` — the API-path row of the matrix below: serial baseline + a 24-way burst exceeding the pg pool, gated on p95 AND an independent superuser COUNT proving every distinct-`client_id` write persisted under contention). First observed numbers on the dev VM (NOT yet prod-baselined): GET /wardrobe full-page serial p95 ≈ 20ms / burst-@24 p95 ≈ 92ms @ ~674 req/s; POST /wear-log serial p95 ≈ 17ms / burst-@24 p95 ≈ 138ms @ ~398 req/s — the full-stack serial p95 is ~2× the DB-only lane, isolating the withAuth+parseBoundary+serialize overhead. STILL MISSING: the teaser time-to-first-preview lane (needs the injected-latency provider transport), the spend-limiter contention lane, the webhook-apply-under-load lane, and the gate-wall self-measurement. Those remain stated budgets, not passing gates.
>
> **⚠️ ORIGINAL STATUS (2026-08-10): there was NO timing harness yet.** The eng-reqs say so twice — `check-budget` counts declared **weights**, not wall time, so "§9's p95 < 90s has no measurement harness behind it" (docs/02:148, docs/02:181), and the teaser parse budget — the single most load-bearing number in the product — is explicitly **unset**: "set a concrete target in the F1 task" (docs/02:181, deferred from docs/01:45). The only runtime numbers that exist are **single observations, not a p95** (fast wall test step ~1.2s, full integration suite ~34s; docs/02:181) and **estimates never observed against a real provider** (~2s provider call docs/06:219; ~137s serial worst case parse-jobs.repo.ts:24-31 — "no adapter has ever received a response", docs/06:226). This tier's job is to **turn every one of those into a measured, gated number.** Until the harness lands, each SLO below is a **stated budget with a NEW/grounded citation**, not a passing gate; building it is tracked the same way the mutation battery is (`LAUNCH-READINESS.md`).
