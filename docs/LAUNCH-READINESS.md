...
7. **Build the on-device privacy-gate classifier + curate the labeled intimate/not-her corpus; prove recall against it on-device.** **[A→H]** — agent builds the model + harness; human curates the corpus and owns the safety go/no-go. **Launch blocker — the privacy invariant is unenforced until this passes its recall floor.**
    ...
    **7b. STRIP EXIF/GPS ON-DEVICE — a hard requirement for whoever writes the upload path.** [A] Verified 2026-08-07: `git grep -ni "exif|gps|geotag"` over `packages/` returns NOTHING, and there is currently **no upload code at all** in `packages/mobile` (no `ImagePicker`, no `MediaLibrary`) — so this is a FUTURE requirement, not a present defect. It is recorded here because the moment an upload path exists it becomes one. iOS camera-roll JPEG/HEIC files carry `GPSLatitude`/`GPSLongitude` and capture timestamps; a wardrobe photo is taken at home, so unstripped EXIF is a **home address** attached to every uploaded file, forwarded to OpenAI + Photoroom and retained indefinitely in the `originals` bucket (which `privacy-policy.md:129` already documents as "retained after processing — not deleted"). Strip metadata BEFORE upload by re-encoding on-device (`expo-image-manipulator` drops EXIF on re-encode) — that is the only fix consistent with the ABLATE-tier invariant, since you cannot leak what never leaves the device. Add a test asserting the uploaded byte stream contains no APP1/Exif marker. **Note for accuracy:** the privacy policy's existing location promise (`privacy-policy.md:93-95`) is about the *weather* feature (coordinates sent device→provider, never to us) — it does **not** currently make a claim about photo metadata, so there is no live policy contradiction today. There would be one the moment photos upload unstripped.
