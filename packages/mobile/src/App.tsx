...
// sanctioned composer).
//
// Provider order: QueryClient (data) → ThemeProvider (tokens) → SessionProvider
// (identity) → ApiProvider (typed client, bearer sourced FROM the session) → the
// session gate → NavShell. SessionProvider must sit ABOVE ApiProvider: the client's
...
export function App(): React.JSX.Element {
  ...
  const port = React.useMemo<AuthPort>(() => makeSupabaseAuthPort(), []);
